...
La planificación debe considerar restricciones reales del taller, pero todavía no se han aprobado catálogo, ancho de corte, pérdidas, tolerancias, remanente mínimo, costos, tiempos, prioridades ni algoritmo definitivo. El dominio debe permitir incorporar esos parámetros versionados sin inventarlos.
...
- `StockPiece` y `PieceSpecification`.
- Remanente y merma.
- `CutRequirement`.
...
- Definir remanente mínimo, costos de máquina, tiempos o prioridades exactas.
...
El producto base representa la identidad comercial/material gobernada por catálogo. Una barra completa y cada corte físico se representan como piezas de stock relacionadas con ese producto base. `PieceSpecification` describe las características necesarias para compatibilidad y uso, pero sus atributos concretos se definirán con el catálogo real.

`StockPiece` conserva identidad, producto base, especificación aplicable, longitud, unidad, origen, ubicación, estado y referencias de trazabilidad. Crear una pieza no crea automáticamente un SKU comercial.
...
El optimizador podrá considerar:
...
`CutOperation` registra la ejecución de un plan o un corte autorizado fuera de plan. Conserva pieza origen, operador/contexto, equipo cuando corresponda, medidas reales, piezas resultantes, remanente, merma, tiempo y diferencias justificadas.

Solo la ejecución confirmada genera movimientos del ADR 0005: consume la pieza origen y da de alta piezas resultantes, remanente y merma mediante eventos balanceados. Si la ejecución difiere del plan, se preservan ambos; no se reescribe el patrón para que coincida con el resultado.

### Remanentes y merma

Un remanente reutilizable es una pieza con origen, longitud medida, ubicación, estado y compatibilidad conservados. Su clasificación depende de un umbral aprobado y versionado. Si no cumple el umbral o se descarta, la merma queda explícita con causa; no se esconde en un ajuste genérico.
...
| Pregunta de implementación          | ¿Cuál es el remanente mínimo por especificación?                             | Bloquea clasificación de remanente.                                 |
...
- Existen remanentes potencialmente reutilizables, pero su umbral está pendiente.
...
- [Registro de decisiones de producto v2.0](../product/decisions/Registro_Decisiones_Producto_v2.0.md), `PROD-020` y `PROD-021`.
