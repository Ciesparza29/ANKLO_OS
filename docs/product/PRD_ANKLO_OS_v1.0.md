...
2. [ADR 0003](../adr/0003-modelo-grupo-entidad-unidad-contexto.md) a [ADR 0008](../adr/0008-inventario-externo-servipernos-y-prestamos.md), en su versión corregida actual y con estado propuesto.
...
Los ADR 0003–0008 siguen siendo propuestas. Su precedencia en este PRD evita recuperar decisiones desplazadas, pero no cambia su estado ni sustituye la ratificación requerida.
...
| Controlar cortes, remanentes y merma           | Balance plan–ejecución y genealogía cuando la fase sea habilitada.             | Meta posterior a casos reales.    |
...
1. Una necesidad autorizada crea `CutRequirement`.
2. El adaptador recibe piezas candidatas y parámetros versionados.
3. `CutPlan` y `CutPattern` preservan entradas, versión y resultado reproducible.
4. Aprobar no modifica inventario.
5. `CutOperation` registra la ejecución real.
6. Solo la ejecución confirmada genera movimientos, piezas, remanente y merma.
7. Una diferencia conserva plan y resultado; no cambia requisitos técnicos de obra.
...
- **Descripción:** relacionar producto base, `StockPiece`, `PieceSpecification`, origen, remanente y merma sin crear SKU por corte ocasional.
- **Actor o capacidad:** inventario y taller autorizado.
- **Precondiciones:** producto/especificación compatibles y pieza propia disponible.
- **Comportamiento esperado y resultado:** cada pieza resultante conserva producto, origen, operación, longitud, unidad, ubicación y estado.
- **Reglas relacionadas:** catálogo estable; propiedad verificada; merma explícita; reversos conforme a inventario.
- **Prioridad, fase y estado:** P1; posterior; confirmado funcionalmente con catálogo pendiente.
- **Fuente:** ADR 0007; `PROD-020`; `LOG-Q-011`, `LOG-Q-019`.
...
- **Prioridad, fase y estado:** P2; posterior; configurable/pendiente.
- **Fuente:** ADR 0007; `PROD-020`; `LOG-Q-016`–`LOG-Q-019`; `IMP-Q-013`; `CFG-006`, `CFG-007`.
...
#### FR-CUT-003 — Ejecución y balance plan–real

- **Descripción:** registrar `CutOperation`, consumo, piezas, remanente, merma y diferencias.
- **Actor o capacidad:** ejecutor de corte y aprobador/conciliador por asignar.
- **Precondiciones:** operación autorizada, pieza reservada y requisitos técnicos vigentes.
- **Comportamiento esperado y resultado:** solo la ejecución confirmada genera movimientos balanceados; plan y real permanecen separados.
- **Reglas relacionadas:** no reescribir plan; no cambiar diámetro, grado, embebido ni diseño de obra.
- **Prioridad, fase y estado:** P1; posterior; confirmado funcionalmente con autoridades pendientes.
- **Fuente:** ADR 0007, con ADR 0005 para movimientos de ejecución; `PROD-020`, `PROD-021`.
...
### 23.2 Informes posteriores

- Parte diario, informe semanal y dossier de cierre.
- Plan versus ejecución de corte, remanente y merma.
...
| Piezas y registro manual de corte                                           | —                                      | Sí                        | —                                 | `POSTERIOR`               | Requiere catálogo y casos controlados.                                                      |
...
| 2. Taller y terceros     | Controlar transformación física y relaciones con inventario externo           | Piezas, registro de corte, optimización, Servipernos, préstamos y liquidaciones           | Catálogo real, parámetros medidos, casos de corte y acuerdos/evidencias de terceros               |
...
| RSK-013 | Merma no registrada durante ejecución                                      | Existencia inflada y costo incompleto            | Registro de piezas, remanente y merma antes del cierre                                  | Por asignar                                        | Abierto; fase posterior                           |
| RSK-014 | Corte calculado con parámetros supuestos o falsos                          | Pérdida material o instrucción insegura          | Parámetros medidos, versión, simulación y validación humana; no optimizar sin evidencia | Autoridad técnica por asignar                      | Abierto; fase posterior                           |
...
| FR-CUT-001–003 — plan y ejecución de corte                      | Principal: ADR 0007. Soporte: ADR 0005 para movimientos y ADR 0006 para requisitos comerciales.                                    | `PROD-020`, `PROD-021`                                     | `LOG-Q-011`, `LOG-Q-012`, `LOG-Q-016`–`LOG-Q-019`, `IMP-Q-013`, `CFG-006`, `CFG-007`                                                                                           | AC-009, AC-010                        |
...
Los ADR 0003–0008 corregidos gobiernan este PRD frente a formulaciones nominales anteriores. La fuente desplazada se conserva como historial; no se reescribe como si nunca hubiera existido. Las autoridades concretas siguen pendientes de una matriz o de evidencia ratificada.
...
- ADR 0001–0008 en `docs/adr/`, con prevalencia de las correcciones vigentes de 0003–0008.
