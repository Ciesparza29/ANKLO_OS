import { cutRequestIdSchema, submitCutRequestSchema } from "@anklo/contracts";
...
export async function POST(
  ...
  try {
    ...
    const input = await parseJson(request, submitCutRequestSchema);
    const result = await getCutRequestService().submit(actor, id, input);
